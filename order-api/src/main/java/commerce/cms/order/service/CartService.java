...
public class CartService {
  ...
  public Cart addCart(Long customerId, AddProductCartForm form) {
    ...
    if(productOptional.isPresent()) { // 장바구니에 같은 상품이 있다.
      Cart.Product redisProduct = productOptional.get();
      List<ProductItem> items = form.getItems().stream() // 새로 추가할 아이템 목록
      ...
      for(Cart.ProductItem item : items) {
        ...
        if(redisItem == null) { // 같은 아이템이 없다.
          redisProduct.getItems().add(item);
        }else {
          if(!redisItem.getPrice().equals(item.getPrice())) {
            cart.addMessage(redisProduct.getName()+" 상품의 "+item.getName()+"의 가격이 변경되었습니다. 확인 부탁드립니다.");
          }
          redisItem.setCount(redisItem.getCount()+item.getCount());
        }
      }
    }else { // 장바구니에 같은 상품이 없음.
